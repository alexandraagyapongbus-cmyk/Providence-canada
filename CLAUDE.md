...
### Brand hierarchy: Providence is the brand, not the underlying carriers

The customer-facing brand relationship is CUSTOMER → PROVIDENCE → INTERNET/TV/PHONE/CONNECTIVITY → CONNECTED. It is never CUSTOMER → PROVIDENCE → BELL/PRIMUS/OTHER CARRIERS. Providence must never read as a comparison site, referral service, or middleman that exists to help customers find *another* telecom company. The customer's mental model should be "I go to Providence for my internet, TV, phone, and telecom needs" — not "I go to Providence to find a telecom provider."

In practice:

- Marketing copy, headings, and navigation are organized around Providence's own services (Internet, TV, Phone, Connectivity, Bundles), never around "providers," "carriers," or comparison-shopping language. Banned patterns for customer-facing copy: "Compare leading providers," "Shop providers," "Our providers," "Leading telecommunications providers," "Choose your provider," "Providers we work with," or any headline/section built around carrier choice as the value proposition.
- There is no "Providers" section in primary navigation. The customer-facing IA is Residential / Business / Solutions (`/canada/solutions`, formerly `/canada/providers` — renamed because "Providers" as a nav label itself signals comparison-site positioning). `/canada/solutions` (`SolutionsPage` in `components/market-pages.tsx`) is organized by Providence's own service categories (Internet, TV, Phone, Connectivity, Bundles), each linking into the relevant Residential/Business page section — not by carrier.
- Carrier/provider disclosure is fine, and sometimes necessary, but only as **product disclosure**, never **brand marketing**: e.g. "which provider serves your area" (address-dependent availability), "vary by provider and plan" (feature-comparison caveat), "your current provider" (switching/porting context), or a single low-key "Providence works with underlying telecommunications providers to deliver these services" disclosure line (see `/canada/solutions`'s "How service is delivered" section). These are factual, contract/billing-adjacent asides, not headlines or value propositions. Before adding or keeping any "provider" mention, ask: **whose brand does this section build — Providence's, or the underlying carrier's?** If it makes a carrier more memorable than Providence without a legal/contractual/disclosure reason, rewrite it.
- The one place carrier/provider-relationship framing is correct and expected is `/canada/telecom` ("Partner With Providence") — see story B below. That page addresses telecom companies themselves as prospective B2B clients, not consumers, so provider-centric language there is the correct framing, not an exception to police.

### Telecom has two distinct, non-overlapping stories — do not blend them

**A. Providence as a telecom storefront/reseller (primary, consumer-facing).** Providence helps residential and business customers get internet, TV, phone, and bundle service, branded as Providence's own offering (see brand hierarchy above). This lives on the Canada homepage, `/canada/residential`, `/canada/business`, `/canada/solutions`, and `/canada/why-providence`.

**B. Providence performing telesales/customer-acquisition work for telecom companies (secondary, B2B).** This is a different service: Providence's own team does outbound sales, representation, lead generation, and campaign execution *for* a telecom company, as a paid service — Providence is not that company's customer-facing storefront in this case. This lives entirely on `/canada/telecom` ("Partner With Providence"), linked from About and the footer, not primary navigation. Provider-centric language ("Telecom provider," "Telecom providers connect with...") is intentional and correct on this page only.
...
- **Do not name, imply a relationship with, or display the logo of any specific real telecom company** (e.g., Bell, Rogers, TELUS, Primus, Zayo) anywhere on the site unless the owner has explicitly confirmed that exact relationship exists. `/canada/solutions` (`SolutionsPage`) and story A generally are intentionally generic — they describe Providence's own service categories, not named carriers. This was explicitly tested with the owner once already (illustrative-only was the confirmed answer); treat any request to add real provider/carrier names as requiring the same explicit confirmation again.
...
Canada routes: `home`, `residential`, `business`, `solutions`, `why-providence`, `healthcare-staffing` (+ alias `services`), `care-organizations`, `healthcare-workers`, `telecom`, `about`, `contact`. Primary nav (`components/market-shell.tsx`) is Home / Residential (dropdown) / Business (dropdown) / Solutions / Why Providence / Healthcare / About / Contact, plus a "Get a Quote" header button linking to `/canada/contact`. The Residential and Business dropdown items are anchors (`#internet`, `#tv`, `#phone`, `#bundles`, `#connectivity`, `#packages`) into their respective single pages, not separate thin pages — keep it that way unless there's real per-category content to justify a split.
