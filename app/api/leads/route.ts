...
const allowedInterests = {
  canada: ['Internet or telecom inquiry', 'Business consultation', 'Telesales support', 'Marketing or sales partnership', 'General inquiry'],
...
export async function POST(request: Request) {
  try {
    ...
    if (email.length > 160 || phone.length > 40 || message.length > 1500) return Response.json({ error: 'One or more fields are too long.' }, { status: 400 });
